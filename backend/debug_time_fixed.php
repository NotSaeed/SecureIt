...
try {
    ...
    $mysqlTime = $db->fetchOne('SELECT NOW() as now_time');
    echo "Current MySQL time: " . $mysqlTime['now_time'] . "\n\n";
    ...
    if (!$send) {
        ...
        if ($sendWithoutTime) {
            ...
            echo "MySQL NOW(): " . $mysqlTime['now_time'] . "\n";
            
            // Compare times
            $expiresTime = new DateTime($sendWithoutTime['expires_at']);
            $currentTime = new DateTime($mysqlTime['now_time']);
            
            if ($expiresTime < $currentTime) {
                echo "*** SEND HAS EXPIRED ***\n";
                echo "Expired " . $currentTime->diff($expiresTime)->format('%h hours, %i minutes ago') . "\n";
            } else {
                echo "Send should still be valid - there might be a timezone issue\n";
            }
        }
    } else {
